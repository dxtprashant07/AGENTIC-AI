...
from .tools.travelPlannerTool import TravelPlannerTool
...
class ToolAgent:
    def __init__(self):
        ...
        self.travel_planner = TravelPlannerTool()
...
Please provide a detailed response combining the search results with additional context and explanation."""
    ...
    def execute_task(self, task):
        ...
        # Extract location and budget dynamically
        import re
        location_match = re.search(r'trip to ([a-zA-Z\s]+)', task_lower)
        location = location_match.group(1).strip() if location_match else "a location"
        budget_match = re.search(r'under ₹(\d+)', task_lower)
        ...
        # Handle travel-related tasks
            ...
            return self.travel_planner.plan_itinerary(location, 3)
        elif 'budget hotels' in task_lower:
            return self.travel_planner.suggest_budget_hotels(location)
        elif 'calculate total cost' in task_lower:
            return self.travel_planner.calculate_total_cost(2000, 3, 5000, budget)
